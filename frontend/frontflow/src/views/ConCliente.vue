<template>
  ...
  <div class="flex flex-col items-center justify-between p-4 space-y-3 md:flex-row md:space-y-0 md:space-x-4">
    <div class="w-full md:w-1/2">
      ...
      <form class="flex-grow max-w-md mr-4"> <!-- Adicionado mr-4 para espaçamento -->
        <label for="default-search" class="mb-2 text-sm font-medium text-gray-900 dark:text-gray-200 sr-only">Search</label>
        <!-- Usando flex para alinhar o input e botões na mesma linha -->
        <div class="relative flex items-center">
          <!-- Ícone de Pesquisa -->
          <div class="absolute inset-y-0 left-0 flex items-center pl-3 pointer-events-none">
            <svg class="w-4 h-4 text-slate-300 dark:text-gray-400" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20">
          ...
          <button
              ...
              class="text-white absolute right-2.5 bottom-2.5 bg-slate-400 hover:bg-slate-200 focus:ring-4 focus:outline-none focus:ring-slate-100 font-medium rounded-lg text-sm px-4 py-2"
            >
              Search
            </button>
        </div>
  ...
  <div class="container px-1 mx-auto">
    <div class="p-4 mb-6 bg-white  overflow-x-auto">
      <table class="table-auto w-full">
        ...
        <tbody>
          <tr v-for="cliente in clientes" :key="cliente.id" class="text-xs bg-gray-50 hover:bg-[#e2e8f0]">
            <td class="py-5 px-6 font-medium">{{ cliente.id }}</td>
            ...
            <td>
              <span class="inline-block py-1 px-2 text-purple-500 bg-purple-50 rounded-full">{{ cliente.email }}</span>
            </td>
            <td>
...
export default {
  ...
  methods: {
    ...
    buscarProdutos() {
      ...
      this.$router.push({ query: { nome: this.busca } });
