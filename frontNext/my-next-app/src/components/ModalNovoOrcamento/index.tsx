...
export default function ModalNovoOrcamento({
  ...
  const handleChange = (
    ...
    if (name === 'servico' || name === 'produto') {
      ...
      setForm(prev => ({ ...prev, observação: value }))
    } else {
      setForm(prev => ({
        ...prev,
  ...
  const funcionariosComCargo = funcionarios.filter(
    f => f.cargo_funcionario != null && f.cargo_funcionario.id != null
  )
